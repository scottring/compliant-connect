...
async function getCompanyDetails(companyId: string | null | undefined) {
...
serve(async (req: Request)=>{
  ...
  let payload: any; // Use 'any' for flexibility or define a more specific union type
  try {
  ...
  if (!sendgridApiKey) {
    ...
    return new Response("Internal Server Error: Email configuration missing", { status: 500, headers: corsHeaders });
  }
  ...
  try {
    ...
    if (payload.type === 'PIR_STATUS_UPDATE') {
      ...
      switch(newStatus){
        case 'draft': // Assuming 'draft' means newly created by customer -> Notify Supplier
          if (supplierEmail) {
            msg = {
              to: supplierEmail, from: senderEmail,
          ...
          } else { console.warn(`No supplier email found for PIR ${pirId}`); }
          break;
        case 'submitted': // Supplier submitted -> Notify Customer
          if (customerEmail) {
            msg = {
              to: customerEmail, from: senderEmail,
          ...
          } else { console.warn(`No customer email found for PIR ${pirId}`); }
          break;
        case 'flagged': // Customer flagged -> Notify Supplier
          if (supplierEmail) {
            msg = {
              to: supplierEmail, from: senderEmail,
          ...
          break;
        case 'approved': // Customer approved -> Notify Supplier
          if (supplierEmail) {
            msg = {
              to: supplierEmail, from: senderEmail,
    ...
    // --- Handle Supplier Invitation (Triggered by Frontend Invoke) ---
      ...
      msg = {
        to: invite_email, from: senderEmail,
    ...
    if (msg) {
      ...
      return new Response(JSON.stringify({ success: false, message: "No email generated for this payload type or condition." }), { status: 200, headers: corsHeaders }); // Return 200 even if no email sent, but indicate why
    }
