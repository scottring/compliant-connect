...
async function serveHttp(conn: Deno.Conn) {
  ...
  for await (const requestEvent of httpConn) {
    ...
    if (requestEvent.request.method !== 'POST') {
      await requestEvent.respondWith(new Response("Method Not Allowed", { status: 405 }));
      continue;
    }

    let payload: EmailPayload;
    try {
      payload = await requestEvent.request.json();
      console.log("Received payload:", payload);
    } catch (e) {
      console.error("Failed to parse request body:", e);
      await requestEvent.respondWith(new Response("Bad Request: Invalid JSON", { status: 400 }));
      continue;
    }
    ...
    if (!sendgridApiKey) {
      ...
      await requestEvent.respondWith(new Response("Internal Server Error: Email configuration missing", { status: 500 }));
      continue;
    }
    ...
    try {
      ...
      if (payload.type === 'PIR_STATUS_UPDATE') {
        ...
        if (newStatus === oldStatus) {
            ...
            await requestEvent.respondWith(new Response("OK: Status unchanged", { status: 200 }));
            continue;
        }
      ...
      if (msg) {
        ...
        await requestEvent.respondWith(new Response("Email sent successfully", { status: 200 }));
      } else {
        console.log("No email message generated for this payload.");
        await requestEvent.respondWith(new Response("OK: No action taken", { status: 200 }));
      }

    } catch (error) {
      console.error("Error processing request:", error);
      await requestEvent.respondWith(new Response(`Internal Server Error: ${error.message}`, { status: 500 }));
    }
  }
}
