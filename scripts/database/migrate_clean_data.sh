...
DATA_MIGRATION_FILE="data_migration.sql"

echo "Creating data migration script for moving data from $DEV_PROJECT_ID to $STAGING_PROJECT_ID..."

# Create a data migration script with SQL queries
cat > "$DATA_MIGRATION_FILE" << EOL
-- Data Migration Script
-- Run these queries in the Supabase SQL Editor

-- First, in the development project (${DEV_PROJECT_ID}), run these queries to export data:

-- Companies
SELECT json_agg(row_to_json(t))
FROM (SELECT * FROM companies) t;

-- Company Relationships
SELECT json_agg(row_to_json(t))
FROM (SELECT * FROM company_relationships) t;

-- Tags
SELECT json_agg(row_to_json(t))
FROM (SELECT * FROM tags) t;

-- Question Sections
SELECT json_agg(row_to_json(t))
FROM (SELECT * FROM question_sections) t;

-- Questions
SELECT json_agg(row_to_json(t))
FROM (SELECT * FROM questions) t;

-- Question Tags
SELECT json_agg(row_to_json(t))
FROM (SELECT * FROM question_tags) t;
...
-- PIR Requests
SELECT json_agg(row_to_json(t))
FROM (SELECT * FROM pir_requests) t;

-- PIR Tags
SELECT json_agg(row_to_json(t))
FROM (SELECT * FROM pir_tags) t;

-- PIR Responses
SELECT json_agg(row_to_json(t))
FROM (SELECT * FROM pir_responses) t;

-- PIR Response Components
SELECT json_agg(row_to_json(t))
FROM (SELECT * FROM pir_response_components) t;

-- PIR Response Component Materials
SELECT json_agg(row_to_json(t))
FROM (SELECT * FROM pir_response_component_materials) t;

-- Response Flags
SELECT json_agg(row_to_json(t))
FROM (SELECT * FROM response_flags) t;

-- Response Comments
SELECT json_agg(row_to_json(t))
FROM (SELECT * FROM response_comments) t;

-- Product Answers
SELECT json_agg(row_to_json(t))
FROM (SELECT * FROM product_answers) t;

-- Product Answer History
SELECT json_agg(row_to_json(t))
FROM (SELECT * FROM product_answer_history) t;

-- Then, in the staging project (${STAGING_PROJECT_ID}), run these queries to import data:
-- Replace the [PASTE_JSON_HERE] placeholders with the JSON data from the export queries
...
SELECT id, customer_id, supplier_id, status::relationship_status, type, created_at, updated_at
...
SELECT id, section_id, text, description, type::question_type, options, required, created_at, updated_at, order_index
...
SELECT id, product_id, customer_id, supplier_company_id, title, description, due_date, created_at, updated_at, suggested_product_name, status::pir_status
...
SELECT id, pir_id, question_id, answer, submitted_at, created_at, updated_at, status::response_status, customer_review_status::response_status
...
SELECT id, response_id, created_by, description, status::flag_status, resolved_at, resolved_by, created_at, updated_at
...
echo "Data migration script created at $DATA_MIGRATION_FILE"
echo "Follow the instructions in the script to manually migrate your data"